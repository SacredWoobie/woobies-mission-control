      ...
      headingActions={
        <>
          ...
          <span aria-label="Pinned note text size" className="notes-font-controls">
            <button aria-label="Decrease pinned note text size" disabled={fontSize <= 12} onClick={() => setFontSize((value) => Math.max(12, value - 1))} type="button">A-</button>
            <button aria-label="Reset pinned note text size" className="notes-font-value" onClick={() => setFontSize(14)} title="Reset to default" type="button">{fontSize}px</button>
            <button aria-label="Increase pinned note text size" disabled={fontSize >= 22} onClick={() => setFontSize((value) => Math.min(22, value + 1))} type="button">A+</button>
          </span>
