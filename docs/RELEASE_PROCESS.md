...
The v0.7.0 release manifest selects:
...
The previous v0.6.1 service set remains frozen in
`tools/Release-Pack-v0.6.1.psd1`. The selected v0.7.0 contract is recorded in
`tools/Release-Manifest.psd1` and frozen in
`tools/Release-Pack-v0.7.0.psd1`.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.0 -GameDataPath $serviceGameData -SkipReleaseImages
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.0 -GameDataPath $serviceGameData
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.0 -GameDataPath $serviceGameData -CreateDraftRelease
```
...
`.zz-90`, so `Woobies-Mission-Control-v0.7.0.zip` remains the first release
