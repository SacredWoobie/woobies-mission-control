...
The v0.7.1 release manifest selects:
...
The previous v0.6.1 and v0.7.0 release contracts remain frozen in
`tools/Release-Pack-v0.6.1.psd1` and `tools/Release-Pack-v0.7.0.psd1`.
The selected v0.7.1 contract is recorded in
`tools/Release-Manifest.psd1` and frozen in
`tools/Release-Pack-v0.7.1.psd1`.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.1 -GameDataPath $serviceGameData -SkipReleaseImages
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.1 -GameDataPath $serviceGameData
```
...
Version 0.7.0 uses five new curated captures from the deterministic development
dashboard because its Flight and Editor surfaces changed materially. Follow
`docs/images/v0.7.0/README.md`; every release image must be a true 1920x889 PNG,
the developer corner control must remain hidden, and the clean packaged runtime
must be validated separately without fixtures or development-only controls.
Version 0.7.1 is a packaging-only hotfix and reuses this unchanged, approved
v0.7.0 screenshot set.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tools\Publish-Release.ps1 -Version 0.7.1 -GameDataPath $serviceGameData -CreateDraftRelease
```
...
`.zz-90`, so `Woobies-Mission-Control-v0.7.1.zip` remains the first release
