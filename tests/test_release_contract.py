...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_v070_release_pack_remains_immutable(self):
        v070_product, v070_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.7.0.psd1"
        )

        self.assertEqual(v070_product, "0.7.0")
        self.assertEqual(v070_services["WoobiesControlStats"], "0.2.21.0")

    def test_v071_release_pack_remains_immutable(self):
        v071_product, v071_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.7.1.psd1"
        )

        self.assertEqual(v071_product, "0.7.1")
        self.assertEqual(v071_services["WoobiesControlStats"], "0.2.21.0")

    def test_v072_release_pack_remains_immutable(self):
        v072_product, v072_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.7.2.psd1"
        )

        self.assertEqual(v072_product, "0.7.2")
        self.assertEqual(v072_services["WoobiesControlStats"], "0.2.21.0")

    def test_v073_release_pack_remains_immutable(self):
        v073_product, v073_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.7.3.psd1"
        )

        self.assertEqual(v073_product, "0.7.3")
        self.assertEqual(v073_services["WoobiesControlStats"], "0.2.21.0")

    def test_v074_release_pack_remains_immutable(self):
        v074_product, v074_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.7.4.psd1"
        )

        self.assertEqual(v074_product, "0.7.4")
        self.assertEqual(v074_services["WoobiesControlStats"], "0.2.21.0")

    def test_v080_release_manifest_records_krpc_060_service_cohort(self):
        manifest = (
        ...
        self.assertIn('ProductVersion = "0.8.0"', manifest)
    ...
    def test_v080_release_pack_matches_the_selected_manifest(self):
        ...
        pack_product, pack_services = read_manifest(
            ROOT / "tools" / "Release-Pack-v0.8.0.psd1"
        )
        ...
        self.assertEqual(manifest_product, "0.8.0")
        self.assertEqual(pack_product, manifest_product)
        self.assertEqual(pack_services, manifest_services)
        self.assertIn("v0.8.0 release manifest selects", release_process)
    ...
    def test_readme_identifies_the_current_public_release(self):
        ...
        self.assertIn("The v0.8.0 public release selects", readme)
        self.assertNotIn("Next release candidate", readme)
        self.assertNotIn("v0.8.0 release candidate", readme)
    ...
    def test_release_inputs_include_current_scene_images(self):
        ...
        screenshot_brief = (
            ROOT / "docs" / "images" / "v0.7.3" / "README.md"
        ).read_text(encoding="utf-8")
        required = re.findall(
            r"\| \d \| (?:not ready|ready|captured|approved) "
            r"\| `([^`]+\.png)` \|",
        ...
        for name in required:
            self.assertIn(f"docs/images/v0.7.3/{name}", publish_script)
            image = (
                ROOT / "docs" / "images" / "v0.7.3" / name
            ).read_bytes()
            self.assertTrue(image.startswith(b"\x89PNG\r\n\x1a\n"), name)
            self.assertEqual(
                struct.unpack(">II", image[16:24]), expected_dimensions[name]
            )
    ...
    def test_release_assets_sort_zip_before_curated_images(self):
        ...
        self.assertEqual(
            image_names,
            [
                "zz-01-mission-overview-catppuccin-portrait.png",
                "zz-02-editor-catppuccin-portrait.png",
                "zz-03-flight-ksp2-navball.png",
                "zz-04-flight-settings-visual-options.png",
                "zz-05-mission-overview-roster-transfers.png",
            ],
        )
        zip_name = "Woobies-Mission-Control-v0.8.0.zip"
        checksum_name = f"{zip_name}.sha256"
        release_image_names = [
            f"Woobies-Mission-Control-v0.8.0.{name}" for name in image_names
        ]
        source_archive_name = (
            "Woobies-Mission-Control-v0.8.0.zz-00-"
            "KRPC.WoobiesMechJeb-0.8.10-source.zip"
        )
        update_name = (
            "Woobies-Mission-Control-v0.8.0.zz-90-runtime-update.zip"
        )
    ...
    def test_new_visual_sources_keep_license_and_acknowledgment_provenance(self):
        ...
        self.assertIn("Version 0.8.0 includes", readme)
